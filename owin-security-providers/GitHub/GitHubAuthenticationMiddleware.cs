...
using Owin;
using owin_security_providers.GitHub.Provider;

namespace owin_security_providers.GitHub
{
            ...
            : base(next, options)
        {
            if (String.IsNullOrWhiteSpace(Options.ClientId))
                throw new ArgumentException(String.Format(
                    "Exception_OptionMustBeProvided {0}", "ClientId"));
            if (String.IsNullOrWhiteSpace(Options.ClientSecret))
                throw new ArgumentException(String.Format(
                    "Exception_OptionMustBeProvided {0}", "ClientSecret"));
